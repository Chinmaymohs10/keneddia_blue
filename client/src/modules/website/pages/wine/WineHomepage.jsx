...
import WineBestSellers from "./components/WineBestSellers";
import WineShowcaseSlider from "./components/WineShowcaseSlider";
import WineNewsSection from "./components/WineNewsSection";
import WineGuestReviews from "./components/WineGuestReviews";
import WineQuickBooking from "./components/WineQuickBooking";
...
// Cities with Kennedia Wine properties — used for matching against Mapbox nearby results
...
export default function WineHomepage() {
  ...
  useEffect(() => {
    if (!isGeolocationAvailable) {
      console.warn("WineHomepage: Geolocation not supported by this browser.");
      return;
    }
    if (isGeolocationEnabled === false) {
      console.warn("WineHomepage: Location permission denied.");
      return;
    }
    if (positionError) {
      console.warn(
        "WineHomepage: Position error —",
        positionError.code === 1 ? "Permission denied" : "Location unavailable"
      );
      return;
    }
    if (!coords) {
      console.log("WineHomepage: Waiting for user coordinates...");
    }
  ...
  useEffect(() => {
    ...
    const fetchNearby = async () => {
      try {
        ...
        // ── Parallel fetch: railway stations + airports + Wines + reverse geocode ──
        const [stationsResult, airportsResult, WinesResult, reverseGeoRes] = await Promise.all([
          ...
          search.category("wine", { proximity: [longitude, latitude], limit: 10 }),
          fetch(
        ...
        // ── Process Wines ────────────────────────────────────────────────────────
        const nearbyWines = WinesResult.features
          .filter((f) => f.properties.distance / 1000 <= NEARBY_RADIUS_KM)
          .map((f) => ({
            name: f.properties.name,
            distanceKm: f.properties.distance / 1000,
  ...
  const handlePopupOk = () => {
    ...
    setTimeout(() => {
      document.getElementById("wine-properties")?.scrollIntoView({ behavior: "smooth" });
    }, 150);
  };
          ...
          <>
            ...
            >
              ...
              <h3 className="text-lg font-serif font-semibold text-foreground">
                Kennedia Wine near you!
              </h3>
              <p className="mt-1.5 text-sm text-muted-foreground">
                We found a Kennedia Wine property in{" "}
