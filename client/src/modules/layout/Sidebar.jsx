...
function Sidebar({ isOpen, onToggle }) {
  ...
  const { coreItems, verticals } = getMenuItemsByRole(userRole);
  ...
  const getRoleBadge = (role) =>
    role === "ROLE_SUPERADMIN" ? "Super Admin" : "Admin";
  const getRoleBadgeColor = (role) =>
    role === "ROLE_SUPERADMIN" ? colors.primary : colors.info;
    ...
    <div key={item.label}>
      ...
      >
        ...
        {item.hasSubmenu && (
          <ChevronDown
            size={16}
            className={`transition-transform shrink-0 ${expandedMenus[item.label] ? "rotate-180" : "rotate-0"}`}
          />
        )}
      </div>
  ...
  return (
    <>
      ...
      <div
        className={`w-60 h-screen flex flex-col fixed left-0 top-0 overflow-y-auto z-40 transition-transform duration-300 ${isOpen ? "translate-x-0" : "-translate-x-full"}`}
      ...
      >
        ...
        <nav className="flex-1 py-5 overflow-y-auto">
          {/* Core Section */}
          <div className="px-5 mb-2 text-[10px] font-bold uppercase tracking-widest text-muted-foreground opacity-50">
            Management
          </div>
          {coreItems.map(renderNavItem)}

          {/* Verticals Section */}
          <div className="px-5 mt-6 mb-2 text-[10px] font-bold uppercase tracking-widest text-muted-foreground opacity-50">
            Verticals
          </div>
          {verticals.map(renderNavItem)}
        </nav>
